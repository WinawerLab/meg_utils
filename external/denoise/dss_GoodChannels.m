% function [dataToDssFull,dataFromDssFull,ratio,pwr,goodChannels]=dss_GoodChannels(x,w,keep1,keep2)
function [todss,fromdss,ratio,pwr,goodChannels]=dss_GoodChannels(x,w,keep1,keep2)
...
[xx,ww]=mean_over_trials_dan(x,w);
ww=min(ww,[],2);
...
badChannels = find(abs(c1(1,:))<keep2);
goodChannels = setdiff(1:size(c1,2),badChannels);
% derive DSS matrix
[todss,fromdss,ratio,pwr]=dss_dan(c0,c1,keep1,keep2);

% dataToDssFull = zeros(size(c1));
% dataFromDssFull = zeros(size(c1));
% 
% dataToDssFull(:,goodChannels) =  todss;
% dataFromDssFull(goodChannels,:) =  fromdss;
