function outputInfo = TSPCA_VibrationSensors(...
    sizeOfBlocks,...
    shifts,...
    sourceFile,...
    excludedChannels,...
...
% the 3 vibrational channels ONLY and then creates a new sqd file with the 
% filtered neuronal data and everything else unmodified. 
% The method is the TSPCA (tsr).
% -----------------------------------------------------------------------------------
...
denoiseChannels = combinedInput(:,190:192);
...
tspca_out = tsr_dan(sensorChannelsIn,denoiseChannels, shifts, weightsByTimeTSPCA);
sns_out = tspca_out;
