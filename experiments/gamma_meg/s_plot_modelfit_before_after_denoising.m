...
%             subplot(10,1,ii)
            ...
            plot(f,10.^model_fit_before(ii,:,chan),'k--', 'LineWidth',4); hold on;
            plot(f,data_before(:,ii,chan), 'color', color_scheme(ii,:,:), 'LineWidth',2);
            
            plot(f,10.^model_fit_before(10,:,chan),'color',rgb_grey,'LineWidth',2);
            ...
            legend('Model fit', 'Data', 'Model fit baseline');
    ...
    for chan = 14
        for ii = 1:9
            ...
            plot(f,10.^model_fit_after(ii,:,chan),'color', cmap(ii,:,:), 'LineWidth',4); hold on;
...
%             hgexport(gcf, fullfile(project_pth, subj_pths{subject_num}, 'figs',sprintf('data_modelfit_after_chan%d_cond%d_2',chan,ii)));
                ...
                plot(f,10.^model_fit_after(10,:,chan),'color',rgb_grey,'LineWidth',2);
