% s_GAMMA_MEG_analysis_1s_trials
...
% data to be analysed
data_pth                      = {'05_Gamma_2_12_2015_subj010'};
...
fs                            = 1000;        % sample rate
epoch_start_end               = [.05 1.050];  % start and end of epoch, relative to trigger, in seconds
...
fit_f2  = NaN(num_conditions,length(t),num_channels); % fitted spectrum

% Fit each channel separately
for chan = data_channels
    % for THIS experiment, blank is condition 10. That need not be true for
    % other experiments
    data_base = spectral_data_mean(:,10,chan);
...
fH = figure(998); clf; set(fH, 'Name', 'Gamma response')
for cond = 1:9
    subplot(3,3,cond)
...
fH = figure(999); clf; set(fH, 'Name', 'Broadband Power')
for cond = 1:9
    subplot(3,3,cond)
